...
import { Trophy, Crown, Award, Home, User } from "lucide-react";
import { Celebration } from "@/components/games/shared";

export default function RummyGameSummaryModal({
    gameData,
    onReturnToLobby,
}: {
    gameData: RummyData;
    ...
    const standings = gameData.playOrder
        .map((pid) => ({
            pid,
            score: gameData.scores?.[pid] ?? 0,
        }))
        .sort((a, b) => b.score - a.score);

    const winnerName = winnerId
        ? (gameData.players?.[winnerId]?.name ?? winnerId)
        : null;

    const lastRound = gameData.lastRoundSummary;
                    ...
                    {isOpen && (
                        <>
                            ...
                            <div className="w-full space-y-3">
                                {standings.map(({ pid, score }, index) => {
                                    const isWinningPlayer = pid === winnerId;
                                    const lastDelta =
                                        lastRound?.perPlayer?.[pid]?.delta;
                                    return (
                                        <motion.div
                                            key={pid}
