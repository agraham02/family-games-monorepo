...
export function DebugEngine() {
    ...
    const [actionLog, setActionLog] = useState<Array<{ event: string; payload: unknown; time: string }>>([]);
    ...
    useEffect(() => {
        ...
        const mockData = gameEntry.generateMockData(options) as unknown as { gameData: GameData; playerDataMap: Record<string, PlayerData> };
    ...
    const handleEmit = useCallback((event: string, payload: unknown) => {
        setActionLog(prev => [{ event, payload, time: new Date().toISOString() }, ...prev]);
        
        const payloadRecord = payload as Record<string, unknown>;
        if (event === "game_action" && payloadRecord?.action) {
            const action = payloadRecord.action as { type: string; payload: Record<string, unknown>; userId?: string };
            ...
            setMasterGameState(prevState => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const newPlayerDataMap = { ...prevState.playerDataMap } as any;
                
                // Basic reducer for Spades PLAY_CARD
                if (action.type === "PLAY_CARD" && selectedGameId === "spades") {
                    const card = action.payload.card as { suit: string; rank: string };
                    const playerId = action.userId || selectedPlayerId;
                    
                    // Remove card from player's hand
                    if (newPlayerDataMap[playerId] && newPlayerDataMap[playerId].hand) {
                        newPlayerDataMap[playerId] = {
                            ...newPlayerDataMap[playerId],
                            hand: newPlayerDataMap[playerId].hand.filter(
                                (c: { suit: string; rank: string }) => !(c.suit === card.suit && c.rank === card.rank)
                            )
                        };
                    }
                    ...
                    if (!newGameData.currentTrick) {
                        newGameData.currentTrick = { plays: [], suit: card.suit };
                    }
                    ...
                    newGameData.currentTurnIndex = (currentIndex + 1) % playOrder.length;
                    ...
                    if (newGameData.currentTrick.plays.length === 4) {
                        ...
                        setTimeout(() => {
                            setMasterGameState(s => {
                                ...
                                const nextState = { ...s };
                                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                const nextGameData = { ...s.gameData } as any;
                                ...
                                // Clear current trick
                                nextGameData.currentTrick = { plays: [], suit: null };
                                
                                // Winner leads next trick (just pick random for debug)
                                nextGameData.currentTurnIndex = Math.floor(Math.random() * 4);
                                
                                return { ...nextState, gameData: nextGameData };
                            });
                        }, 1500);
                    }
                }
                ...
                if (action.type === "PLACE_BID" && selectedGameId === "spades") {
                ...
                if (action.type === "PLAY_TILE" && selectedGameId === "dominoes") {
                    const tile = action.payload.tile as { left: number; right: number };
                    ...
                    if (newPlayerDataMap[playerId] && newPlayerDataMap[playerId].hand) {
                        newPlayerDataMap[playerId] = {
                            ...newPlayerDataMap[playerId],
                            hand: newPlayerDataMap[playerId].hand.filter(
                                (t: { left: number; right: number }) => !(t.left === tile.left && t.right === tile.right)
                            )
                        };
                    }
                    ...
                    if (!newGameData.board) {
                        newGameData.board = { tiles: [], leftEnd: null, rightEnd: null };
                    }
                ...
                if (action.type === "ROLL_DICE" && selectedGameId === "lrc") {
                    const playOrder = newGameData.playOrder;
                    const currentIndex = newGameData.currentTurnIndex;
                    newGameData.currentTurnIndex = (currentIndex + 1) % playOrder.length;
                }
                ...
                return {
                    ...
                    playerDataMap: newPlayerDataMap
                };
            });
        }
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            console.log(`[AutoPlay] Attempting to play for ${currentTurnPlayerId}`);
            
            if (selectedGameId === "spades" && gameData.phase === "playing") {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const playerData = masterGameState.playerDataMap[currentTurnPlayerId] as any;
            ...
            } else if (selectedGameId === "spades" && gameData.phase === "bidding") {
                // Auto-bid
                handleEmit("game_action", {
                    roomId: "debug-room",
                    action: {
                        type: "PLACE_BID",
                        payload: { bid: { amount: 3, type: "normal", isBlind: false } },
                        userId: currentTurnPlayerId
                    }
                });
            } else if (selectedGameId === "dominoes" && gameData.phase === "playing") {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const playerData = masterGameState.playerDataMap[currentTurnPlayerId] as any;
                if (playerData && playerData.hand && playerData.hand.length > 0) {
            ...
            } else if (selectedGameId === "lrc" && gameData.phase === "playing") {
                handleEmit("game_action", {
                    roomId: "debug-room",
                    action: {
                        type: "ROLL_DICE",
    ...
    const players = useMemo(() => {
        ...
        return Object.values(masterGameState.gameData.players).map((p: { id: string; name: string }) => ({
            id: p.id,
            name: p.name
        }));
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden bg-background">
            <MockSessionProvider initialUserId={selectedPlayerId} initialRoomId="debug-room">
